...
// Wait, the SVG for Claude Opus 4.6 in the actual code might be slightly different than my snippet.
// Let's use regex to remove it safely.
code = code.replace(/<div className="mb-4 flex items-center justify-between">[\s\S]*?<h2 className="text-xl font-bold text-white leading-tight">Comparatif des prix<br\/>API<\/h2>[\s\S]*?<\/div>[\s\S]*?<div className="space-y-3 mb-6">[\s\S]*?<div className="bg-gradient-to-r from-\[#122426\] to-\[#0D181C\] border border-\[#10B981\]\/10 p-4 rounded-xl flex items-center justify-between mb-3">[\s\S]*?<\/div>[\s\S]*?<div className="bg-\[#111823\] border border-white\/5 p-4 rounded-xl flex items-center justify-between">[\s\S]*?<\/div>[\s\S]*?<\/div>/g, "");
