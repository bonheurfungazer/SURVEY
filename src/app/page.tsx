...
const countryCodeToFrenchName: Record<string, string> = {
  "AF": "Afghanistan",
  "ZA": "Afrique du Sud",
  "AX": "Ahvenanmaa",
  "AL": "Albanie",
    ...
    const init = async () => {
      ...
      try {
          ...
          if (data && data.country) {
              const frenchName = countryCodeToFrenchName[data.country] || data.country_name;
              setVoteForm(prev => ({
                  ...prev,
                  country: frenchName,
        ...
        <div className="w-full min-h-screen relative">
            ...
            <div className="relative px-6 pt-6 pb-12 overflow-hidden flex flex-col items-center">
                ...
                <div className="glass-hero w-full rounded-3xl p-8 relative z-10 flex flex-col items-center text-center overflow-hidden">
                    ...
                    {!user ? (
                        <button onClick={() => {setShowLoginModal(true); setIsSignUp(true);}} className="w-full bg-[#3B82F6] hover:bg-blue-600 text-white font-bold py-4 rounded-xl text-sm transition-all duration-200 active:scale-95 relative z-10 btn-primary-glow mb-6 flex items-center justify-center space-x-2">
                            <i className="fas fa-user-plus"></i>
                            <span>Créer un compte pour voter</span>
                        </button>
                    ) : (
                        <button onClick={() => setCurrentTab('vote')} className="w-full bg-[#10B981] hover:bg-[#047857] text-white font-bold py-4 rounded-xl text-sm transition-all duration-200 active:scale-95 relative z-10 shadow-[0_0_20px_rgba(16,185,129,0.4)] mb-6 flex items-center justify-center space-x-2">
                            <span>Voter maintenant</span>
                            <i className="fas fa-arrow-right"></i>
                        </button>
                    )}

                    <div className="w-full bg-[#1A2332]/80 border border-[#3B82F6]/30 rounded-xl p-4 mb-8 relative z-10 flex items-center shadow-[0_0_15px_rgba(59,130,246,0.1)]">
                        <div className="w-10 h-10 rounded-lg bg-[#3B82F6]/20 flex items-center justify-center shrink-0 mr-4">
                            <i className="fas fa-terminal text-[#3B82F6] text-lg"></i>
                        </div>
                        <div className="text-left flex-1">
                            <h3 className="text-white font-bold text-sm leading-tight mb-1">Authentifiez vos CLI</h3>
                            <p className="text-[#94A3B8] text-[10px] leading-snug">
                                Utilisez la clé API avec <span className="text-[#E2E8F0] font-semibold">Claude Code</span>, <span className="text-[#E2E8F0] font-semibold">Gemini CLI</span> ou <span className="text-[#E2E8F0] font-semibold">Codex</span> pour coder directement depuis votre terminal.
                            </p>
                        </div>
                    </div>
